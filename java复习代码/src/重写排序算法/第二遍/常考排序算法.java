...
public class 常考排序算法 {
    /**
     * 1.冒泡排序：适用于数据量不大，对稳定性有要求，且数据基本有序的情况
 ...
 */
    ...
    public int[] selectSort(int[] arr) {
        ...
        for (int i = 0; i < arr.length; i++) {
            ...
            //此时只需将该最小值与第一个元素arr[i]交换即可，使第一个元素成为最小值
    ...
    private int partition(int[] arr, int left, int right) {
        ...
        //中轴值随意选，一般就选最左边那个元素，将数组元素中小于该值的元素放左边，大于该值的元素放右边
        //最后，将中轴元素也插入其中，即双指针重合的位置（是要与中轴元素所选的元素的位置互换，而不是强插进来）
        int pivot = arr[left];
        int i = left;
        int j = right;
        while (i < j) {
            //1.开始按照中轴值，将小于该值的所有元素放左边，大于该值的所有元素放右边
            //此时说明位置i处的值本来就比中轴值小，因此直接跳过该值，i++，进行下一个值的比较
            while (i <= j && arr[i] <= pivot) {
                i++;
            }
            //2.同理，此时说明位置j处的值本来就比中轴值大，
            //因此直接跳过该值，j--，进行下一个值的比较
