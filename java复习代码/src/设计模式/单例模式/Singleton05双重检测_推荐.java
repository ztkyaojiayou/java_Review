 ...
 * <p>
 * ➢优缺点说明:
 * 1)优点:这种写法比较简单,I就是在类装载的时候就完成实例化，避兔了线程同步问题。
 ...
 * * 了这个判断语句，这时便会产生多个实例
 * （即：此时另一个线程先创建了，但之前那个对象也会继续执行并创建对象，因此会产生两个对象实例）。
 * 所以在多线程环境下不可使用这种方式
 ...
 * 执行getInstance0方法都要进行同步。
 * 而其实这个方法只执行一次实例化代码就行，后面的想获得该类实例，直接return就行了。
 * 易知，该方式的效率太低
 ...
 * 这时t2突然获取了CPU执行权（因为这里没有加锁，因此这种现象是完全有可能的），
 * 也执行了第一次校验，判断也为null，且若t2先获得了锁，于是就会先创建该单例对象的实例，创建完之后，释放锁。
...
class Singleton05 {
    ...
    public static Singleton05 getInstance() {
        ...
        if (instance == null) {//这一步是还没有加锁的，因此多个线程可能都通过该逻辑
            //3.2 加锁，目的是为了保证多线程下的安全性问题，易知，若不加锁，则多个线程都有可能先判断为null，
            //然后又分别去创建对象（因为只要判断完null后，后面就不会再次判断了），从而导致创建了多个对象。
            synchronized (Singleton05.class) {//锁的是该单例对象，这里面肯定只能是一个线程去创建对象，但要考虑到此时可能有线程2也在这里等了!!!
                //3.3 第二次检测，目的是为了防止“多个线程通过了第一重null检测后（相当于最开始），
                // 再分别加锁，分别创建对象，从而又创建了多个对象”的现象。
                //也即：线程1先创建了对象，线程2由于也先同时跳过了第一重检测，若不再次进行检测，则会又创建一个新对象，
                // 而由于之前线程1已经创建了对象，本不应该再次创建的，此时就不再是单一对象啦！！！
                if (instance == null) {//主要针对多线程情形
                    //3.4 开始真正创建对象。
