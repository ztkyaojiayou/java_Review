...
public class demo05_00二叉树的三种遍历方式 {
    ...
    public List<Integer> preOrderTraversal(数据结构与算法.TreeNode root) {
        List<Integer> res = new ArrayList<>();
        if (root == null) {
            return res;
        }
        Stack<数据结构与算法.TreeNode> stack = new Stack<>();
        stack.push(root);

        while (!stack.isEmpty()) {
            数据结构与算法.TreeNode current = stack.pop();
            res.add(current.val);
     ...
     * @return
     */
    public List<Integer> postOrderTraversal(数据结构与算法.TreeNode root) {
        List<Integer> list = new ArrayList<>();
        Deque<数据结构与算法.TreeNode> stack = new ArrayDeque<>();
        数据结构与算法.TreeNode node = root;
