...
public class 递归_机器人的运动范围 {
    ...
    private static int dps(int k, int rows, int cols, int cur_row, int cur_col, boolean[] visited) {
        int cur_position = cur_row * cols + cur_col;//当前行*总列数+当前列
        if (cur_col < 0 || cur_col > cols || cur_row < 0 || cur_row > rows || visited[cur_position] || !checkSum(k,cur_row,cur_col)){
            return 0;
        }
