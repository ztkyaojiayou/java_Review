 ...
 *
 * 先建立一个相同规格的二维数组dp。
 * 其中的每一个元素 dp(i,j) 表示以 (i,j) 为右下角，且只包含 1 的正方形的边长最大值。
 * 如果我们能计算出所有dp(i,j) 的值，那么其中的最大值即为矩阵中只包含 1 的正方形的边长最大值，其平方即为最大正方形的面积。
 *
 * 那么如何计算 dp 中的每个元素值呢？对于每个位置 (i,j)，检查在矩阵中该位置的值：
 * 如果该位置的值是 0，则 dp(i, j) = 0，因为当前位置不可能在由 1 组成的正方形中；
 * 如果该位置的值是 1（也即这是前提），则 dp(i,j) 的值由其上方、左方和左上方的三个相邻位置的 dp 值 dp(i−1, j)，dp(i, j−1)，dp(i−1, j−1)决定。
 * 具体而言，当前位置的元素值等于三个相邻位置的元素中的最小值加 1（画图可知），状态转移方程如下：
 *
...
public class 最大正方形221 {
        public int maximalSquare(char[][] matrix) {
            ...
            for (int i = 0; i < rows; i++) {//使用双重循环进行遍历
                for (int j = 0; j < columns; j++) {
                    if (matrix[i][j] == '1') {//易知这是前提，若该值不是1，而是0的话，则该值不可能构成全1正方形嘛！（对于为0的情况则可以直接跳过）
                        //3.1初始情况
                        if (i == 0 || j == 0) {
                            dp[i][j] = 1;
