...
public class 编辑距离72 {
        public int minDistance(String word1, String word2) {
             ...
             */
            //3.确定初始/边界情况（即第一行和第一列的情况）
            //3.1第一行：是 word1 为空变成 word2  的前 j 个字符所需要的最少步数，易知，此时即为从无到有，直接使用插入操作即可
            for (int j = 1; j <= n2; j++) {
                dp[0][j] = dp[0][j - 1] + 1;
            }
            //3.2第一列：是 word2 为空，即把word1 的前i个字符变成空字符串所需要的最少步数，易知直接使用删除操作即可
            for (int i = 1; i <= n1; i++) {
                dp[i][0] = dp[i - 1][0] + 1;
            }

            //4.再考虑一般情况，使用状态方程转化为初始情况即可
            for (int i = 1; i <= n1; i++) {
                for (int j = 1; j <= n2; j++) {
            ...
            return dp[n1][n2];//把字符串的长度传入即可得到结果
        }
    }
