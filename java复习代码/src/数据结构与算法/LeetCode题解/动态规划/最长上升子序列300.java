...
/**
 * 给定一个无序的整数数组，找到其中最长上升子序列(不要求相邻）的长度。
 *
 * 示例:
 *
 ...
 * 空间复杂度 O(N) ： dp 列表占用线性大小额外空间。
 ...
 那么我们可以将第 i 个数接到第 j 个数字的后边作为一个新的上升子序列，此时对应的上升子序列的长度就是 dp[j] + 1。
...
public class 最长上升子序列300 {
        public int lengthOfLIS(int[] nums) {
            ...
            //要取max是因为可能有多个满足条件nums[j] < nums[i]的nums[j]，我们需要选一个最大的。
            for (int i = 0; i < nums.length; i++) {
                for (int j = 0; j < i; j++) {
                    if (nums[j] < nums[i]) {//这是前提条件，即前一个数必须比后一个数小才构成上升序列
                        dp[i] = Math.max(dp[i], dp[j] + 1);//在所有满足该条件的dp[j] + 1中选一个最大值存入dp[i]中}
                    }
                    //第一个指针i每移动一位就先更新一下result，直到遍历结束，则所求结果即为最终的result
