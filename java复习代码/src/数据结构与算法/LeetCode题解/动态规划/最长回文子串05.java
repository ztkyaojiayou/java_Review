 ...
 * 即：在头尾字符相等的情况下，里面子串的回文性质决定了整个子串的回文性质，这就是状态转移。
 ...
 *
 * 第 5 步：考虑优化空间
 * 因为在填表的过程中，只参考了左下方的数值。事实上可以优化，
 * 但是增加了代码编写和理解的难度，丢失可读和可解释性。在这里不优化空间。
 *
...
public class 最长回文子串05 {
        public String longestPalindrome(String s) {
            ...
            //再定义一个二维数组/表格，其中dp[i][j] 表示 s[i, j] 是否是回文串，i和j都可取到
            ...
            //由于我们定义的i和j分别代表字串的左边界和右边界，因此i<j，也因此，只需要填写该表格的上半部分即可
            for (int j = 1; j < len; j++) {
                for (int i = 0; i < j; i++) {
