 ...
 */
     ...
     * 1）我们目前持有一支股票（即没有卖，则也就没有冷冻期的说法呀），对应的「累计最大收益」记为f[i][0]；
     * 2）我们目前不持有任何股票，并且处于冷冻期中（即昨天卖了一只股票，那么现在就不能买，因为处于冷冻期），对应的「累计最大收益」记为 f[i][1]；
     * 3）我们目前不持有任何股票，并且不处于冷冻期中（即前天卖了一只股票，那么现在可以买，但没买），对应的「累计最大收益」记为 f[i][2]。
     *
     ...
     *
     * 1）对于 f[i][0]，有两种情况，
     * 1.1我们目前持有的这一支股票可以是在第i−1 天就已经持有的（即没有卖），对应的状态为 f[i−1][0]；
...
class 高阶版123 {
        public int maxProfit(int[] prices) {
            ...
            //第一次交易最大利润
            int maxProfit1 = 0;
            //第二次买入后的最大剩余利润,初始化为最小值
            int maxProfitAfterBuy = Integer.MIN_VALUE;
            //(第二次后）当天最终能获得最大最大利润
            int maxProfit2 = 0;
            //遍历每一天的股票价格，更新上述四个状态值
