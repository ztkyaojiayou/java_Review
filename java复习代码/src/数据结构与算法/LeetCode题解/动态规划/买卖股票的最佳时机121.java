...
class 入门级121 {
        public int maxProfit(int[] prices) {
            ...
            int max_profit = 0;
            //1.2记录买入时机，初始化为第一天的值，
            int min_price = prices[0];//由于数组的索引是从0开始的，因此这里写0，而不是1。
            //2.开始遍历数组
...
class 进阶版122 {
    public int maxProfit(int[] prices) {
        ...
        for (int i = 1; i < prices.length; i++) {
            int tmp = prices[i] - prices[i - 1];//表示当天的利润（可能为负）
 ...
 * 题目中定义的“冷冻期”=卖出的那一天的后一天，题目设置冷冻期的意思是，如果昨天卖出了，今天不可买入，那么关键在于哪一天卖出，
 * 只要在今天想买入的时候判断一下前一天是不是刚卖出，即可，所以关键的一天其实是卖出的那一天，而不是卖出的后一天
 *
 * 正文：
 ...
 * 重头戏：
 *
 * 一、第i天不持股且没卖出的状态dp[i][0]，也就是我没有股票，而且还不是因为我卖了它才没有的，
 * 那换句话说是从i-1天到第i天转移时，它压根就没给我股票！所以i-1天一定也是不持有，
 * 那就是不持有的两种可能：i-1天不持股且当天没有卖出dp[i-1][0]；i-1天不持股但是当天卖出去了dp[i-1][2]；
 * 所以： dp[i][0]=max(dp[i-1][0],dp[i-1][2])
 *
 * 二、第i天持股dp[i][1]，今天我持股，来自两种可能：
 * 1、要么是昨天我就持股，今天继承昨天的，也就是dp[i-1][1]，这种可能很好理解；
 * 2、要么：是昨天我不持股，今天我买入的，但前提是昨天我一定没卖！
 * 因为如果昨天我卖了，那么今天我不能交易！也就是题目中所谓“冷冻期”的含义，
 * 只有昨天是“不持股且当天没卖出”这个状态，我今天才能买入！所以是dp[i-1][0]-p[i]
 * 所以： dp[i][1]=max(dp[i-1][1],dp[i-1][0]-p[i])
 *
 * 三、i天不持股且当天卖出了，这种就简单了，那就是说昨天我一定是持股的，
 * 要不然我今天拿什么卖啊，而持股只有一种状态，
 * 昨天持股的收益加上今天卖出得到的新收益，就是dp[i-1][1]+p[i]啦
 * 所以：dp[i][2]=dp[i-1][1]+p[i]
 *
 * 总结：对于所求结果，最后一天的最大收益有两种可能，而且一定是“不持有”状态下的两种可能，把这两种“不持有”比较一下大小，返回即可
 */
class Solution309 {
    public int maxProfit(int[] prices) {
        int n=prices.length;
        ...
        //1.定义一个二维数组,定义三种状态：
        //（1）dp[i][0]表示第i天不持股且当天没卖出时的最大收益;
        //（2）dp[i][1]表示第i天持股时的最大收益；
        //（3）dp[i][2]表示第i天不持股且当天卖出了时的最大收益；
        int [][] dp=new int[n][3];
        ...
        for(int i=1;i<n;i++){//从[1]...[n-1]
            dp[i][0]=Math.max(dp[i-1][0],dp[i-1][2]);//不持股且当天没卖出时的最大收益
            dp[i][1]=Math.max(dp[i-1][1],dp[i-1][0]-prices[i]);//持股时的最大收益
            dp[i][2]=dp[i-1][1]+prices[i];//不持股且当天卖出了时的最大收益
 ...
 * 第二次卖出后的状态： maxProfit2: 当天能获得最大最大利润
 *
 ...
 *
 * 其中，如果理解 maxProfitAfterBuy ？ 即： 第二次买入后的最大剩余利润
