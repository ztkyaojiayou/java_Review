...
public class 戳气球312 {
        public int maxCoins(int[] nums) {
            ...
            return dpCount(dp, new_nums, 0, n - 1);
        }

        //真正的求法
        public int dpCount(int[][] dp, int[] nums, int left, int right) {
            //确定初始情况
            if (left + 1 == right)
                return 0;
            ...
            for (int i = left + 1; i < right; i++){
                ...
                dp[left][right] = Math.max(dp[left][right], nums[left] * nums[i] * nums[right]
                        + dpCount(dp, nums, left, i) + dpCount(dp, nums, i, right));
            }
