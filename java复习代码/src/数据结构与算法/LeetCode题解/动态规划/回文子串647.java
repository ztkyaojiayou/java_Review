...
public class 回文子串647 {
        public int countSubstrings(String s) {
            ...
            int result = s.length();//结果值初始化为该字符串的长度值，
            ...
            //3.再看一般情况，使用状态方程
            for (int j = 1; j < s.length(); j++) {//使用遍历，即以j结尾，依次判断前面的字符串是否为回文子串，
                // 如：若j = 5，则判读（0,5），（1,5）...(4,5)的情况
                for (int i = 0; i < j; i++) {
                    if(s.charAt(i) == s.charAt(j)) {//3.1只有当两头的字符相等时，才有可能是回文子串，
                        //即只需考虑这种情况即可（否则肯定不是回文子串（默认就是false））

                        //3.2特殊情况，即若i和j之间没有元素了（即相邻），则就不能通过递推关系式计算啦，
                        //此时要其两头都相等，则最终的结果也是回文子串。
                        if(j-i == 1){
                            dp[i][j] = true;
                        }
                        else{//否则，就可以转化到其里面区间（即区间（i+1，j-1）的字符串是否是回文子串
                            dp[i][j] = dp[i+1][j-1];
                        }
                    }
                    //4.每判断完一个字符串，就在结果中记一笔
