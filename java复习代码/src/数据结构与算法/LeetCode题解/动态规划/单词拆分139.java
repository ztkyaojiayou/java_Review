...
public class 单词拆分139 {
        public boolean wordBreak(String s, List<String> wordDict) {
           //1.把字典表存入HashSet中
            ...
            //4.考虑一般情况，使用状态方程
            for(int i = 1; i<=s.length(); i++){//下标i用于考虑所有从当前字符串开始的可能的子字符串
                for(int j = 0; j<i; j++){//通过下标j把每一个子字符串又拆分成两个子字符串（即前j个字符构成的字符串s1和由j至i所构成的字符串s2），以此类推，直到转化为初始情况
                    if(dp[j] && set.contains(s.substring(j,i))){//若这两个子字符串都在字典表中，则让dp[i]为true，否则令其为false。
