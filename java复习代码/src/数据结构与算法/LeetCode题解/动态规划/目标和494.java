 ...
 * 找到数组nums中的一个子集P,使得sum(P) = (目标S + 数组和) / 2  (易知为一个定值）
 *
 ...
 * （1）当0<=j<w[i]时，说明当前的第i件物品放不进背包里，那么要想填满容量为j的背包，只能从前i-1件物品里去选，所以其方案数继承自G[i-1][j]；
 * （2）当j>=w[i]时，说明第i件物品可以放进背包里，此时就有两种互斥的情况，即可以放入i，也可以不放入i，最后把这这两种情况的方法数加起来即可，分析如下：
