 ...
 * 一般来说关于DFS有两个套路：
 * 第一个就是用递归来完成，这种在理解上着实也不是很简单，但是一旦理解了，起飞！
 * 第二个的话就是用stack来完成遍历，会比上面好理解，但是复杂度会稍微差一点。
 *
 * 思路：
 * 此题就是一个明显的递归问题，我们要从root开始，逐步的先往左枝走，如果不满足再倒退回上一个节点，再往右枝走。
 ...
 * 关于为什么取并集，因为只要有一个path存在，我们的任务就完成了，所以只要有true，最后就是true。
 */
class 路径总和1_判断_112{
    public boolean hasPathSum(TreeNode112 root, int sum) {
        ...
        //2.递归结束的条件
        // 也即确定一个符合要求的结果,返回true（也同时起到了剪枝的效果？）
        ...
        //最后返回结果，只要左右子树存在一条路径即符合题意，就返回true，因此使用“或”符号||即可。
        return left || right;
    }
}
...
class 路径总和3_返回总路径数_437 {
    ...
    public int countPath(TreeNode112 root, int sum){
        ...
        //再使用递归来得到当前节点的左右子节点的符合要求的所有路径
        //这里务必注意：每一个头结点都有左右子节点，都要算进去，
        //另外，每个节点都要充当头结点，因此会有上面的2）和3）这两次递归，千万不要混淆
