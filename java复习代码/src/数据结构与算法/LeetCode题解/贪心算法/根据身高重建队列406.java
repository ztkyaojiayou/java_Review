 ...
 * k是排在这个人前面且身高大于或等于自己身高h的人数，（即比它高的人必须站前面，这也是我们需要按照身高降序排列的原因）。
...
public class 根据身高重建队列406 {
        public int[][] reconstructQueue(int[][] people) {
            ...
            //此时，数组people已经按照指定的规则排好了序
                //定义一个输出list（后面再转成数组即可；那为什么不直接定义一个数组呢？因为数组是固定的，无法进行添加操作），开始把人往里面塞
            List<int[]> output = new LinkedList<>();
            //
            for(int[] p : people){//表示按行遍历二维数组people，即把该二维数组看成是由很多行构成的一维数组p，
                 ...
                 */
                output.add(p[1], p);//p[1]:即为每一组值的k，p即为每一组值；把p这一组值插入到list的索引为p[1]的地方
            }

            int n = people.length;//行数
            //转化成一个n行2列的数组输出，即为所求
