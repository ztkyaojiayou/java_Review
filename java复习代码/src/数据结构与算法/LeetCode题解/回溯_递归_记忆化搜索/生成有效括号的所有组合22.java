...
public class 生成有效括号的所有组合22 {
     ...
     * @param usedLeft   左括号已经用了几个
     * @param usedRight  右括号已经用了几个
     * @param n      左括号、右括号一共可以用几个（即题给的n）
     * @param result    结果集
     */
    private void dfs(String curStr, int usedLeft, int usedRight, int n, List<String> result) {
        ...
        //一般情况，使用递归
        if (usedLeft < n) {//若左括号还没有用完，则向左递归，
            // 此时递归函数的具体变化就是：curStr在原字符串上加一个左括号，使用了的左括号left加一，右括号不变
            dfs(curStr + "(", usedLeft + 1, usedRight, n, result);
        }
        if (usedRight < n) {//若右括号还没有用完，向右递归，
            // 此时递归函数变为：curStr在原字符串上加一个右括号，使用了的右括号right加一，左括号不变
            dfs(curStr + ")", usedLeft, usedRight + 1, n, result);
        }
    }
}
