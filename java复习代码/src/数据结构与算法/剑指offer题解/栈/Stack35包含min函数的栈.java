...
public class Stack35包含min函数的栈 {
    ...
    private Stack<Integer> dataStack = new Stack<>();//定义一个普通栈，用于入栈和出栈操作
    ...
    public void push(int node) {
        //1.1直接入栈到dataStack即可
        dataStack.push(node);
        //1.2但同时要把较小元素存入（push）到minStack栈中
        //当minStack.isEmpty()时即插人第一个元素，之后每push之前都必须与栈顶元素进行比较，取最小值插入(注意：相同元素不会被覆盖，也是照样存入）
