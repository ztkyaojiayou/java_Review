...
class 常考排序算法 {
    /**
     * 4.快速排序（常考）
     * 基本思路：先任选一个元素作为划分值（但一般都选第一个值），
     * 通过一次遍历和比较将待排序的数组分割成独立的两部分，
    ...
    public int[] MergeSort(int[] arr) {
        ...
        //1.先把原数组分成左右两边
        int[] leftArr = Arrays.copyOfRange(arr, 0, mid);//左边数组
        int[] rightArr = Arrays.copyOfRange(arr, mid, arr.length);//右边数组
        //2.再对左右两边的数组进行排序(使用递归）
        int[] left_sorted = MergeSort(leftArr);
        int[] right_sorted = MergeSort(rightArr);
        //3.最后再对排好序的数组进行归并即可
        return merge(left_sorted, right_sorted);
    }

    //3.0归并的具体实现（此时两个目标小数组已经排好序啦）
    private int[] merge(int[] sorted_arr1, int[] sorted_arr2) {
        ...
        for (int index = 0; index < res_arr.length; index++) {
            ...
            //则把它依次全部存入res_arr数组中
            if (i >= sorted_arr1.length) {//3.1.1此时即左边走完了，右边还剩了数据(务必加上等号）
                res_arr[index] = sorted_arr2[j];
                j++;
            } else if (j >= sorted_arr2.length) {//3.1.2此时即右边走完了，左边还剩了数据(务必加上等号）
                res_arr[index] = sorted_arr1[i];
                i++;
                //3.2再处理一般情况，即当两边还都有数据时，则先把较小值先存入。
                //3.2.1若左边的当前数据大于右边的当前数据时，则先把右边的数据先存入
 ...
 */
    ...
    /**
     * 3.插入排序：适用于数据量不大，对算法的稳定性有要求，且数据局部或者整体基本有序的情况
