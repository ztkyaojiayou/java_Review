...
public class tree14按之字形顺序打印二叉树 {
     ...
     // （也即在牛客系统的后台只会传入根节点）
    public  ArrayList<ArrayList<Integer>> Print(TreeNode pRoot) {//传入根节点
        ...
        Queue<TreeNode> queue = new LinkedList<>();//把所有结点先存入此队列中（先进先出）
        ...
        while (!queue.isEmpty()) {
            ...
            while (cnt > 0) {//退出循环时就表示这一层已经处理完了
                TreeNode node = queue.poll();//删除并返回队列头部的第一个元素，这里强调返回，且是顺序返回/弹出
                if (node == null) {
