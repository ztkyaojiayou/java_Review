 ...
 * 路径可以从矩阵中任意一格开始(因此需要使用双重循环），每一步可以在矩阵中间向左、右、上、下移动一格。
...
public class 矩阵中是否存在目标路径75 {
    ...
    public static boolean hasPath(char[] matrix, int rows, int cols, char[] str) {
        ...
        //2.表示已经处理的str中字符个数，当该值等于目标字符串的长度时就说明查找成功
        int count = 0;
        //3.以每一个点为起始，调用回溯函数进行深度优先搜索，只要沿着某一个点找到了就返回true
        for (int i = 0; i < rows; i++) {
            for (int j = 0; j < cols; j++) {
                if (hasPathMethod(matrix, rows, cols, str, visited, i, j, count)) {
                    return true;
                }
            }
        }
     ...
     * @param count   已经处理的str中字符个数
     * @return 是否找到 true是，false否
     */
    private static boolean hasPathMethod(char[] matrix, int rows, int cols, char[] str, boolean[] visited,
                                         int cur_row, int cur_col, int count) {
        ...
        if (cur_row >= 0 && cur_row < rows
                ...
                && matrix[i] == str[count]//2）当前字符和目标字符相等
            ...
            count++;//2.同时使“已经处理的str中字符个数加1

            //2.2做选择，按左上右下进行回溯，只要有一个位置
            res = hasPathMethod(matrix, rows, cols, str, visited, cur_row, cur_col - 1, count)//向左
                    || hasPathMethod(matrix, rows, cols, str, visited, cur_row - 1, cur_col, count)//向上
                    || hasPathMethod(matrix, rows, cols, str, visited, cur_row, cur_col + 1, count)//向右
                    || hasPathMethod(matrix, rows, cols, str, visited, cur_row + 1, cur_col, count);//向下
