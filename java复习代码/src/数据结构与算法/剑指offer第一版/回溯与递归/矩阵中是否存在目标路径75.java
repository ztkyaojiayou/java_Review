...
public class 矩阵中是否存在目标路径75 {
    ...
    public static boolean hasPath(char[] matrix, int rows, int cols, char[] str) {
        ...
        for (int i = 0; i < rows; i++) {
            for (int j = 0; j < cols; j++) {
                if (hasPathCore(matrix, rows, cols, str, visited, i, j, count)) {
                    return true;
                }
            }
        }
    ...
    private static boolean hasPathCore(char[] matrix, int rows, int cols, char[] str, boolean[] visited,
        ...
        if (cur_row >= 0 && cur_row < rows
            ...
            res = hasPathCore(matrix, rows, cols, str, visited, cur_row, cur_col - 1, count)//向左
                    || hasPathCore(matrix, rows, cols, str, visited, cur_row - 1, cur_col, count)//向上
                    || hasPathCore(matrix, rows, cols, str, visited, cur_row, cur_col + 1, count)//向右
                    || hasPathCore(matrix, rows, cols, str, visited, cur_row + 1, cur_col, count);//向下
